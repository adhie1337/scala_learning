...
class FunSetSuite extends FunSuite {
  ...
  test("singleton has only one element") {
    new TestSets {
      assert(!FunSets.toString(s1).contains(','))
    }
  }
  ...
  test("diff contains elements of first set without elements of second set") {
    new TestSets {
      val s = union(s1, s2)
      val d = diff(s, s2)
  ...
  test("filter contains same elements of each set") {
    new TestSets {
      val s = union(s1, s2)
      val i1 = filter(s, s1)
      val i2 = filter(s, s2)
